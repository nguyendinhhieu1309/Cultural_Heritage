...
def re_sample(image, current_spacing, new_spacing):
    ...
    # actual_resize_factor = np.flip(new_shape / image.shape)
    ...
    #image_resized = dip.Resampling(image, zoom=actual_resize_factor, interpolationMethod='nn')
    # image_resized = np.asarray(image_resized, dtype=np.float32)

    return image_resized, new_spacing


def re_sample_shape(image, current_spacing, new_shape):
    # actual_resize_factor = np.flip(new_shape / image.shape)
    actual_resize_factor = new_shape / image.shape
    new_spacing = current_spacing / actual_resize_factor
...
def crop(complete):
    dim = complete.shape
    # Along x-axis
    idx_x = ~(complete == 0).all((1, 2))
    complete = complete[idx_x, :, :]

    # Along y-axis
    idx_y = ~(complete == 0).all((0, 2))
    complete = complete[:, idx_y, :]

    # Along z-axis
    idx_z = ~(complete == 0).all((0, 1))
    complete = complete[:, :, idx_z]

    return complete, idx_x, idx_y, idx_z, dim
...
def padding(complete):
    ...
    return complete, dim_x, dim_y, dim_z


def reverse_padding(implant, dim_x, dim_y, dim_z):
    implant = implant[:, :, dim_z[0]:512-dim_z[1]]
    implant = implant[:, dim_y[0]:512-dim_y[1], :]
    implant = implant[dim_x[0]:512-dim_x[1], :, :]

    return implant


def reverse_crop(implant, idx_x, idx_y, idx_z, shape):
    implant_z = np.zeros((implant.shape[0], implant.shape[1], shape[2]))
    implant_z[:, :, idx_z] = implant

    implant_y = np.zeros((implant.shape[0], shape[1], shape[2]))
    implant_y[:, idx_y, :] = implant_z

    implant_x = np.zeros(shape)
    implant_x[idx_x, :, :] = implant_y
