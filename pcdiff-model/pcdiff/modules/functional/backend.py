...
if sys.platform == 'win32':
    # Add PyTorch's library directory to PATH.
    # This helps find DLLs like c10.dll, torch_cpu.dll, torch_cuda.dll, etc.
    torch_lib_path = os.path.join(os.path.dirname(torch.__file__), 'lib')
    if os.path.exists(torch_lib_path) and torch_lib_path not in os.environ['PATH']:
        print(f"Appending PyTorch lib path to os.environ['PATH']: {torch_lib_path}")
        os.environ['PATH'] = torch_lib_path + os.pathsep + os.environ['PATH']

    # Add CUDA Toolkit's bin directory to PATH.
    # This helps find CUDA runtime DLLs like cudart64_*.dll, nvrtc64_*.dll, etc.
    # Try to get CUDA_HOME from environment variable
    ...
    if cuda_home:
        ...
        default_cuda_path_version = "12.5" # From your build log
        ...
        if os.path.exists(default_cuda_base):
            ...
            print(f"Warning: CUDA_HOME or CUDA_PATH environment variable not found. Using fallback CUDA path: {cuda_bin_path}")
        else:
            print(f"Warning: CUDA_HOME or CUDA_PATH environment variable not found, and fallback CUDA path {default_cuda_base} does not exist.")
